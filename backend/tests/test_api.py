...
def test_vehicle_crud():
    ...
    response = httpx.post(f"{BASE_URL}/vehicles/", json=payload, headers=headers)
    assert response.status_code == 201, f"Expected 201, got {response.status_code}. Response: {response.text}"
    data = response.json()
    assert data["registration_number"] == unique_reg
    vehicle_id = data["id"]
    
    response = httpx.get(f"{BASE_URL}/vehicles/{vehicle_id}", headers=headers)
    assert response.status_code == 200
    
    response = httpx.put(f"{BASE_URL}/vehicles/{vehicle_id}", json={"odometer": 100}, headers=headers)
    assert response.status_code == 200
...
def test_driver_crud_and_compliance():
    ...
    response = httpx.post(f"{BASE_URL}/drivers/", json=payload, headers=headers)
    assert response.status_code == 201, f"Expected 201, got {response.status_code}. Response: {response.text}"
    driver_id = response.json()["id"]
    
    response = httpx.put(f"{BASE_URL}/drivers/{driver_id}", json={"status": "On Trip"}, headers=headers)
...
def test_trip_dispatch_logic():
    ...
    resp = httpx.post(f"{BASE_URL}/trips/", json=t_payload, headers=headers)
    ...
    v_resp = httpx.get(f"{BASE_URL}/vehicles/{vehicle_id}", headers=headers)
    assert v_resp.json()["status"] == "On Trip", "Vehicle lock failed"
    d_resp = httpx.get(f"{BASE_URL}/drivers/{driver_id}", headers=headers)
    assert d_resp.json()["status"] == "On Trip", "Driver lock failed"
    
    resp2 = httpx.post(f"{BASE_URL}/trips/", json=t_payload, headers=headers)
    assert resp2.status_code == 400, "Dispatch allowed double-booking an asset"
    
    resp = httpx.put(f"{BASE_URL}/trips/{trip_id}/complete?actual_distance=120", headers=headers)
